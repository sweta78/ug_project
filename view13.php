...
function out_error($error)
{
 ...
 if(empty($error)===false)
 {
	...
	echo"<a href='view3.php'><b><center><h2> FILL THE DETAILS AGAIN </a>WITH PROPER VALUES :-)</h2></center></b>";
}
else
{ 
 $retval=mysql_query("select * from rstatus3 where ROOMNO=\"$roomno\" and SEATNO=\"$seatno\" and NAME=\"$name\" ") or die("could not retrieve");
  ...
  while($sql=mysql_fetch_array($retval, MYSQL_ASSOC)) 
	{
		...
		$retval1=mysql_query("select * from pd3 where UET_ROLL_NO=\"$uet_roll_no\"   ");
	...
	<td>$year</td>
	 ...
	 $retval2=mysql_query("select * from cd3 where UET_ROLL_NO=\"$uet_roll_no\"   ");
