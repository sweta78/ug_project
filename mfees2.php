...
if(1 === preg_match('/\D/',$roomno) )
 ...
 if(empty($error)===false)
 {
	...
	echo"<a href='mform2.php'><b><center><h2> FILL THE DETAILS AGAIN </a>WITH PROPER VALUES :-)</h2></center></b>";
}
else
{ 
  $retval2 = mysql_query("select * from rstatus2 where ROOMNO='$roomno' and SEATNO='$seatno' and NAME= '$name'");
 $row2=mysql_numrows($retval2);
 
 if($row2!=0)
 {
 $sql1="INSERT INTO mess2(ROOMNO,SEATNO,NAME,DAYS,EXTRA_CHARGE,DESCRIPTION) VALUES('$roomno','$seatno','$name','$days','$extra_charge','$description')";
 $retval = mysql_query($sql1) or die(mysql_error());
 
 $put1=mysql_query("update mess2 set FEES=((DAYS*70)+EXTRA_CHARGE)");
 
 $retval1=mysql_query("select * from mess2 where roomno=\"$roomno\" and seatno=\"$seatno\" and name=\"$name\" ") or die("could not retrieve");
 ...
 echo("<b><center><h1><u>MESS FEES CALCULATION</u></h1></center></b> <br/><p>");
	...
	<br/>
	 ...
	 <center>
	 <b>
	<a href='mform2.php'>CALCULATION OF MESS FEES FOR ANOTHER STUDENT</a></b></center>
